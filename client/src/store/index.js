...
export default new Vuex.Store({
  ...
  actions: {
    ...
    getMemberRoom ({ state, commit }, payload) {
      let member = []
      db.collection('dotaFighter').doc(payload).get()
        .then(doc => {
          let list = doc.data()
          member.push(list['player1'])
          member.push(list['player2'])
        })
        .catch(err => {
          console.log(err)
        })
      commit('SET_MEMBER_ROOM', member)
    }
  },
  modules: {
  }
